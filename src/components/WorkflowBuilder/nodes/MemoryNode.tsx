...
import { Database, Settings, AlertCircle, Save, Download, Plus } from 'lucide-react';
      ...
      {(!data.hasOutgoing && data.onAddNode) && (
        <div className="absolute top-full left-1/2 transform -translate-x-1/2 flex flex-col items-center">
          ...
          <button
            ...
            className="w-8 h-8 bg-white border-2 border-gray-300 rounded-full flex items-center justify-center hover:border-orange-500 hover:bg-orange-50 transition-all duration-200 shadow-sm hover:shadow-md"
          >
            <Plus className="w-4 h-4 text-gray-500 hover:text-orange-600" />
          </button>
        </div>
      )}
    </div>
